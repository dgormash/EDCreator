...
namespace FDCreator.Logic
{
    ...
    {
        protected StabilizerParsedData TransferedData = new StabilizerParsedData();
        ...
        public override ParsedData GetPdfData()
        {
            ...
            TransferedData.SerialNumber = Parser.GetStringValueFromRegion(File, rect);

            //Length shoulder to shoulder
            rect = new iTextSharp.text.Rectangle(148, 640, 167, 647);
            TransferedData.Length = Parser.GetStringValueFromRegion(File, rect);

            //Stabilizer OD
            rect = new iTextSharp.text.Rectangle(308, 640, 325, 647);
            TransferedData.StabilizerOd = Parser.GetStringValueFromRegion(File, rect);

            //Lobe Length
            rect = new iTextSharp.text.Rectangle(313, 632, 320, 638);
            TransferedData.LobeLength = Parser.GetStringValueFromRegion(File, rect);

            //Lobe Width
            rect = new iTextSharp.text.Rectangle(315, 624, 318, 630);
            TransferedData.LobeWidth = Parser.GetStringValueFromRegion(File, rect);

            //Fishing neck/Tong space
            rect = new iTextSharp.text.Rectangle(467, 523, 484, 530);
            TransferedData.FishingNeckTongSpace = Parser.GetStringValueFromRegion(File, rect);
            ...
            TransferedData.ConnectionOne = FillConnectionInfo(connectionColumn);
            ...
            TransferedData.ConnectionTwo = FillConnectionInfo(connectionColumn);

            //Version 
            rect = new iTextSharp.text.Rectangle(174, 34, 284, 40);
            TransferedData.Version = VersionExtractor.GetVersion(Parser.GetStringValueFromRegion(File, rect));
